...
Esquema actualizado el 18-09-2026, tras completar el resumen final de parejas. Es una propuesta cuya organización final se hablará con el usuario; **no es un manuscrito ni un envío**. [ROBUSTNESS_RESULTS.md](ROBUSTNESS_RESULTS.md), el piloto y [FIELD_PAIR_RESULTS.md](FIELD_PAIR_RESULTS.md) se complementan. La auditoría y el estado de cierre se consultan en [NEXT_STEPS.md](NEXT_STEPS.md).

**Actualización tras la revisión previa al manuscrito:** incorporar [PREPAPER_REVIEW.md](PREPAPER_REVIEW.md), [CASE_ATLAS.md](CASE_ATLAS.md) y la [bibliografía canónica](references/README.md). La comparación vigente de antecedentes está en [references/RELATED_WORK.md](references/RELATED_WORK.md); añade Singh 2022, Bascur 2025, SemCSE-Multi 2026 y estabilidad de nodos de septiembre de 2026. Las cifras anteriores se conservan.

## Recomendación tras el piloto de morfología, 18-09-2026

**Cierre de las 325 parejas:** priorizar ahora [el gráfico de recuentos y magnitudes](reports/field_pair_summary_v1/01_conclusions_and_magnitude.pdf). Apertura: 262 contradicciones persistentes de 325; PR: 221. A 5% relativo quedan 96 y 177. Explicar que basta con dos modelos opuestos, que los otros pueden coincidir o estar sin resolver y que no se están midiendo errores semánticos. Mantener la sensibilidad al centrado y el panel de seis. El resumen completo sustituye al ejemplo aislado como apoyo de frecuencia; el ejemplo conserva su función explicativa.

Incorporar una sección breve sobre qué descripciones geométricas dependen del encoder, con [el mapa de posiciones de las áreas](reports/morphology_pilot_v1/02_field_rank_maps.pdf) y el contraste ilustrativo Artes/Medicina de [MORPHOLOGY_RESULTS.md](MORPHOLOGY_RESULTS.md). La inversión persiste en muestras y alternativas, pero la receta/procesamiento condicionan otras conclusiones. Es una ampliación exploratoria posterior, sin reemplazar CKA/vecinos.

Para contener la extensión, propongo conservar cuatro figuras principales sustituyendo la figura temporal de la propuesta anterior por esa comparación de propiedades; la comprobación temporal completa quedaría en material adicional. Esto es una propuesta de presentación, no un manuscrito ya redactado. Evitar una segunda narrativa sobre fragmentación temática: los diagnósticos de conexión y sus contraejemplos quedan como comprobaciones adicionales. No interpretar PR como temas ni apertura como diversidad semántica; no ocultar sus cuatro alertas o la dependencia de CLS/SEP/centrado.
...
| 4. Conclusiones y magnitud | La dirección de comparaciones entre disciplinas depende del encoder; separar inversiones pequeñas y mayores. | [01_conclusions_and_magnitude.pdf](reports/field_pair_summary_v1/01_conclusions_and_magnitude.pdf) |
...
- Figura 4: 325 parejas de las 26 áreas; diez modelos, principal de 2.000/área más veinte medias muestras y cinco selecciones de 2.000 del corpus. Contradicción requiere dos modelos de signo opuesto, cada uno persistente en esas condiciones; unanimidad exige los diez. Las filas añaden mínimos de diferencia relativa simétrica, no umbrales de significación. Casos no resueltos permanecen. Apertura no es diversidad temática ni PR número de temas; el seguimiento y el ejemplo son posteriores al piloto. Las parejas comparten áreas/modelos y no son pruebas independientes.

## Tablas y suplemento

Añadir [las 325 parejas visibles](reports/field_pair_summary_v1/02_all_field_pairs.pdf), [las posiciones originales](reports/morphology_pilot_v1/02_field_rank_maps.pdf) y los controles de [FIELD_PAIR_RESULTS.md](FIELD_PAIR_RESULTS.md). El centrado global conserva los mismos testigos en 145/262 aperturas y 218/221 PR: límite sustantivo para el texto, no solo una nota escondida en suplemento. La alternativa de seis modelos y los cortes de magnitud tampoco deben omitirse de la explicación.

Conservar la [figura temporal anterior](reports/robustness_v2/final/figures/04_time_controls.pdf) en el suplemento: forma con tamaños iguales aumenta entre extremos en 24/26 áreas, pero solo cuatro crecen en todos los pasos. Vecinos cambia −0,87 puntos con todos los candidatos y +0,91 con 2.048. Separar el pequeño efecto de consultas del cambio de candidatos. Son controles posteriores al resultado inicial, sin conclusión histórica causal. No confundir ese acuerdo temporal entre modelos con los descriptores temporales nuevos del piloto de morfología.

Añadir una tabla breve de relaciones concretas: enlace persistente entre trabajos de sirtuinas y enlace dependiente del modelo entre trabajos de películas finas/cerámicas. Los doce casos completos se conservan en [el atlas legible](reports/prepaper_v1/CASES.md). Presentar su selección como exploratoria, con candidatos siempre elegibles. Incluir también los errores de fuente encontrados; no usarlos como ejemplos de desacuerdo científico legítimo. El gráfico de posiciones de Subfields puede acompañar la figura de vecinos o ir al suplemento según el espacio.

Para la figura de ámbitos, aclarar que las 50 consultas pertenecen al mismo Subfield y están incluidas en ambos conjuntos de 256. Para los ejemplos de centros, indicar que no se probó otra receta. No prometer una validación temática humana que no existe.
...
La decisión histórica de no añadir descriptores en [METRICS.md](METRICS.md) fue ampliada por la petición expresa del piloto. [MORPHOLOGY_PROTOCOL.md](MORPHOLOGY_PROTOCOL.md) fija apertura, dimensión efectiva lineal y conexión como propiedades distintas, sin sumar indicadores ni llamarlos dimensión intrínseca verdadera. [MORPHOLOGY_RESULTS.md](MORPHOLOGY_RESULTS.md) conserva las alternativas que fallan y limita la incorporación al argumento principal.

## Continuación

Con R01–R12, el piloto y el resumen de parejas verificados, el siguiente paso es hablar de la organización con el usuario. Cuando pida redactar, usar [METHODS_ROBUSTNESS.md](METHODS_ROBUSTNESS.md), los dos métodos anteriores, [METHODS_MORPHOLOGY.md](METHODS_MORPHOLOGY.md) y [METHODS_FIELD_PAIRS.md](METHODS_FIELD_PAIRS.md). No ampliar automáticamente las entradas a 500.000 ni repetir inferencia por abrir otro chat. Un análisis adicional necesita una pregunta y alcance nuevos.
