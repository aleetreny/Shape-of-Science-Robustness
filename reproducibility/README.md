...
These checks passed directly from the archives, with no imports of the original scientific programs. The compact check also passed from an isolated temporary directory. All archived files passed decompressed checksum validation. The PDFs were rebuilt exactly from extracted editable-source packages.
