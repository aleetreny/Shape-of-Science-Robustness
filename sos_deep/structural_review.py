...
BASE=ROOT/'data/robustness_v2';OUT=BASE/'structural_review'
...
def size_and_stability():
    ...
    for measure,rows,column in [('shape',shapes,'cka_debiased'),('neighbors',neighbors,'mean_overlap')]:
        for population in ['all_eligible','same_183_subfields']:
            ...
            for r in aggregate(subset,keys,column):output.append(dict(population=population,measure=measure,k='',**r))
...
def main():
    ...
    with run_lock(OUT):
        ...
        freeze(OUT,['sos_deep/structural_review.py','sos_deep/artifacts.py','sos_deep/control_review.py'],
