...
def write(item, content, selection):
    editorial = OUT / 'tables/editorial' / (item + '.tex')
    if editorial.exists():
        content = editorial.read_text()
        caption = re.search(r'\\captionof\{table\}\{([^}]+)\}', content).group(1)
        header_line = next(line for line in content.splitlines() if line.startswith(r'\textbf') and ' & ' in line)
        headers = re.findall(r'\\textbf\{([^}]+)\}', header_line)
        MANIFEST[item]['rendered_parts'] = [{'caption': caption, 'headers': headers,
                                           'rows': len([line for line in content.splitlines() if ' & ' in line]) - 1}]
        MANIFEST[item]['editorial_source'] = {'path': str(editorial.relative_to(OUT)), 'sha256': sha(editorial)}
        selection = ('Question-first explanation reviewed on 20 September 2026; final robustness repetitions '
                     'and controls are specified in S9. Model settings are unchanged.')
...
def supplementary_tables():
    ...
    assets=[
        ...
        ('Final robustness','reports/robustness_closure_v1/catalog.json','Local extension; no data deposit','See ROBUSTNESS_CLOSURE_REPORT.md')]
    rr=[]; records=[]
    ...
    write(item,content,'Nine manifest SHA-256 values and truthful access states; this is an inventory, not a new data release.')
...
if __name__ == "__main__":
    from build_closure_tables import render
    render(OUT)
