"""Paired model versus input effects on the nested 52k study; no production edits."""
import csv
import itertools
...
from sos_followup.pilot_statistics import cosine_grams,cka_matrix,rsa_matrix,effect_rows

ROOT=Path(__file__).resolve().parents[1]
CONFIG=ROOT/'config/input52_v1.json';D=json.loads(CONFIG.read_text())
INPUTS=ROOT/'data/robustness_v2/inputs';OUT=ROOT/'data/robustness_v2/input_comparisons'
...
def field_run(field,positions,metadata,arrays):
    ...
    n=D['per_cell']*len(D['temporal']['periods'])
    assert len(ids)==n and all((periods==p).sum()==D['per_cell'] for p in D['temporal']['periods'])
    ...
    for i,j in COMPARISONS:
        ...
        records.append({'field_id':field,'model_a':a,'input_a':ca,'model_b':b,'input_b':cb,'kind':'model' if ca==cb else 'input','rows':n,'cka_debiased':float(ck[i,j]),'rsa_spearman':float(rsa[i,j])})
    write_csv(out/'shape.csv',records)
    neighbors=[];verified=0
    for name,x in zip(NAMES,selected):
        nn,info=exact_neighbors(x,ids,k=50)
        anchor=np.linspace(0,n-1,10,dtype=int)
    ...
    for k in [10,25,50]:
        ...
        for i,j in COMPARISONS:
            ...
            a,ca=NAMES[i];b,cb=NAMES[j]
            records.append({'field_id':field,'k':k,'model_a':a,'input_a':ca,'model_b':b,'input_b':cb,'kind':'model' if ca==cb else 'input','mean_overlap':overlap,'chance_adjusted':(overlap-k/(n-1))/(1-k/(n-1))})
        matrices[k]=mat
    ...
    for repeat in range(D['stability']['repeats']):
        ...
        chosen=np.sort(np.concatenate([rng.choice(np.flatnonzero(periods==p),D['stability']['subset_per_period'],replace=False) for p in D['temporal']['periods']]))
    ...
    for effect in [r for r in effects if r['metric']=='cka']:
        ...
        stability.append({'field_id':field,'model':effect['model'],'input_condition':effect['input_condition'],'effect_full2000':effect['model_minus_input'],
              'median_effect1000':float(np.median(vals)),'absolute_median_change':error,'central95_width1000':width,
    ...
    for condition in ['title','abstract']:
        ...
        stability.append({'field_id':field,'model':'mean_of_ten_fixed_models','input_condition':condition,'effect_full2000':float(full),
              'median_effect1000':float(np.median(vals)),'absolute_median_change':error,'central95_width1000':width,
              'passes_operational_screen':error<=D['stability']['max_median_effect_change'] and width<=D['stability']['max_central95_width']})
...
def main():
    with run_lock(OUT):
        ...
        source_names=['sos_deep/input_analysis52.py','sos_followup/pilot_statistics.py','sos_analysis/geometry.py','sos_analysis/neighbors.py','sos_embed/storage.py','requirements-analysis.txt']
        manifest={'schema_version':1,'design_sha256':file_sha(CONFIG),'input_manifests':parents,'source_files':{n:file_sha(ROOT/n) for n in source_names},'representations':[list(n) for n in NAMES],'unit':'2000 papers per Field; 400 each period','comparisons_per_field':165,
        ...
        write_json(OUT/'audit.json',{'all_complete':True,'counts':counts,'independent_neighbor_queries':verifications,'fields':26,'rows_per_field':D['per_cell']*len(D['temporal']['periods']),'parent_manifests_verified':True,'completed_at':utcnow(),'files':{p.name:file_sha(p) for p in sorted(OUT.glob('*.csv'))}})
        write_json(OUT/'progress.json',{'state':'complete','updated_at':utcnow()})
        print('PAIRED 52K INPUT ANALYSIS COMPLETE',flush=True)
