...
def main_tables():
    ...
    for r in models:
        ...
        model = r'\textbf{' + tex(r['display_name']) + r'}\newline{\footnotesize\nolinkurl{' + r['repo_id'] + '}}'
    ...
    content = table(item, 'Embedding models and primary representation settings.',
                    ['Model and checkpoint', 'Dim.', 'Limit', 'Readout', 'Reference'], rows, [6.9, 1.05, 1.1, 2.0, 3.5])
    content += note('Limits count tokens including special symbols. Mean: last-layer average over non-padding positions, '
...
def supplementary_tables():
    ...
    content = table(item, 'Clean corpus by Field, period and cohort.',
             ...
             rows, [4.2, 1.38, 1.38, 1.38, 1.38, 1.38, 1.38, 1.38])
    ...
    for r in fits:
        ...
        rr.append([r'\nolinkurl{' + r['outcome'] + '}', r['rank'] + '/' + r['columns'],
    ...
    write(item, content, 'All eight model-trait fits, with counts of saved non-identifiable omission coefficients; all model traits included.')
