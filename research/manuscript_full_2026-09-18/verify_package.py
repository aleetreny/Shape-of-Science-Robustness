...
def run():
    ...
    assert sha(ROOT/'research/manuscript_opening_2026-09-18/closure_audit.json')==baseline['previous_closure_sha256']
    ...
    assert sha(ROOT/'references/references.bib')==baseline['canonical_bib_sha256']
    assert (MAN/'references.bib').read_text()==(ROOT/'references/references.bib').read_text().replace('Same data—different results?', 'Same data---different results?')
    documents={}
    ...
    result={'created_at':datetime.now(timezone.utc).isoformat(),'numeric_and_integrity_complete':True,
        ...
        'scientific_recomputation':False,'full_manuscript_prose_written':True,
