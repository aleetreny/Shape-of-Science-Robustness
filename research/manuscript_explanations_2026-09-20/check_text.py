...
for cmd in ['citep','citet','label','ref','input']:
 ...
 for lang,folder in [('en','manuscript'),('es','manuscript_es')]:
  ...
  assert re.findall(pattern,old)==re.findall(pattern,texts[lang]),(lang,cmd)
...
for folder in ['manuscript','manuscript_es']:
 ...
 assert {k:v for k,v in old.items() if k!='T01'}=={k:v for k,v in manifest.items() if k!='T01'}
 assert old['T01']['sources']==manifest['T01']['sources']
for lang,folder in [('en','manuscript'),('es','manuscript_es')]:
