...
def run():
    ...
    extracted = ROOT / 'tmp/pdfs/comments_portable_es_source'
    build = ROOT / 'tmp/pdfs/comments_portable_es_build'
    if extracted.exists():
    ...
    (EVIDENCE / 'portable_es_audit.json').write_text(json.dumps(evidence, indent=2) + '\n')
