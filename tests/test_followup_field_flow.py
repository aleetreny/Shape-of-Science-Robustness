...
import sos_followup.input_analysis as flow
...
class KnownInputFlow(unittest.TestCase):
    def test_identical_inputs_preserve_shape_neighbors_and_resume(self):
        ...
        data={m:rng.normal(size=(1000,8+i)).astype(np.float32) for i,m in enumerate(flow.MODELS)}
        arrays={(m,c):data[m] for m,c in flow.NAMES}
        meta=pa.table({'row_index':np.arange(10000,11000),'period_start':np.repeat([2000,2005,2010,2015,2020],200)})
        design={**flow.D,'stability':{**flow.D['stability'],'repeats':2}}
        with tempfile.TemporaryDirectory() as td,patch.object(flow,'OUT',Path(td)),patch.object(flow,'D',design):
            flow.field_run(11,np.arange(1000),meta,arrays)
