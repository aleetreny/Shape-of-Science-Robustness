...
Final robustness|||Robustez final
...
english_manifest=json.loads((ROOT/'manuscript/tables/manifest.json').read_text())
for key, record in english_manifest.items():
    manifest.setdefault(key, record)['sources'] = record['sources']
for source_csv in (ROOT/'manuscript/tables/data').rglob('*.csv'):
    dest=OUT/source_csv.relative_to(ROOT/'manuscript'); dest.parent.mkdir(parents=True,exist_ok=True); shutil.copyfile(source_csv,dest)
for src in sorted((ROOT/'manuscript/tables/tex').glob('*.tex')):
    if src.stem in {'S15', 'S18', 'S19', 'S20'}: continue
    ...
    if editorial.exists():
        ...
        manifest[src.stem]['summary_rule']='Explicación por finalidad revisada el 20-09-2026; cierre de robustez y repeticiones en S9; reglas de modelos sin cambios.'
        continue
