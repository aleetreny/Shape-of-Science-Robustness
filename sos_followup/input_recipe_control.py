...
from .input_analysis import verified_inputs,NAMES,COMPARISONS,MODELS,INPUTS,OUT as PRIMARY
from .pilot_statistics import cosine_grams,cka_matrix,effect_rows

ROOT=Path(__file__).resolve().parents[1]
CONFIG=ROOT/'config/checklist_input_recipes_v1.json';R=json.loads(CONFIG.read_text())
...
def field_run(field,positions,metadata,arrays):
    ...
    write_json(target/'commit.json',{'field_id':field,'rows':1000,'independent_neighbor_queries':proof_count,
...
def main():
    with run_lock(OUT):
        ...
        names=['sos_followup/input_recipe_control.py','sos_followup/input_analysis.py','sos_followup/pilot_statistics.py',
               'sos_analysis/neighbors.py','sos_analysis/geometry.py','requirements-analysis.txt',
               'config/checklist_input_recipes_v1.json','config/checklist_v1.json']
        ...
        print('INPUT RECIPE CONTROLS COMPLETE',flush=True)
