...
ROOT=Path(__file__).resolve().parents[1];INP=ROOT/'data/checklist_v1/inputs';OUT=ROOT/'research/checklist_2026-09-17/input_audit'
...
def audit(model,condition):
    ...
    exact=0;truncated=0;tokens_total=0;lost_total=0
    for shard in sorted((folder/'shards').glob('[0-9]*')):
        ...
        arrays={pool:np.load(shard/(pool+'.npy'),allow_pickle=False) for pool in model['poolings']} if native else None
        for j,(r,o,m,txt,tok,orig_tokens) in enumerate(zip(batch,orig,meta,texts,ids,full)):
            ...
            if native:
                ...
                exact+=1
        saved.extend(meta)
    assert len(saved)==26000
    write_json(target,{'all_pass':True,'completed_at':utcnow(),'model':model['key'],'condition':condition,'rows':len(saved),
        'manifest_sha256':file_sha(p),'input_sha256':file_sha(meta_path),'native_exact_reuse':exact,'truncated_rows':int(truncated),
