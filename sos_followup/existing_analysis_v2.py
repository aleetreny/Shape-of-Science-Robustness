...
OUT=ROOT/'data/checklist_v1/existing_v2'
...
def fit(y,x,allow_rank_deficient=False):
    rank=np.linalg.matrix_rank(x)
    if rank<x.shape[1] and not allow_rank_deficient:raise ValueError('Rank deficient family design')
...
def families(arrays):
    ...
    for name in keep_names:
        ...
        for m in MODELS:
            keep=np.array([m not in pair for pair in PAIRS]);rank=int(np.linalg.matrix_rank(x[keep]));b2,r22=fit(y[keep],x[keep],allow_rank_deficient=True)
            if rank==4:
                deviations.append(b2)
                held.extend((y[~keep]-x[~keep]@b2)**2);base.extend((y[~keep]-y[keep].mean())**2)
            for j,n in enumerate(names):loo.append({'outcome':name,'omitted_model':m,'feature':n,'coefficient':float(b2[j]) if rank==4 else None,'r_squared':r22,'design_rank':rank,'coefficients_identified':rank==4})
        ...
        summ[name]={'coefficients':dict(zip(names,map(float,beta))),'r_squared':r2,'reference_tail_fraction':tail,
          ...
          'leave_one_models_identified':len(deviations),'leave_one_model_prediction_improvement_over_training_mean':float(1-sum(held)/sum(base)),'prediction_scope':'identified deletions only; rows without unique coefficients excluded and reported'}
...
def main():
    with run_lock(OUT):
        ...
        write_json(OUT/'audit.json',{'all_complete':True,'outcomes':len(arrays),'values_per_outcome':5850,
            'source_tables_verified':True,'family_full_design_rank':4,'unidentified_deletion':'SEP recipe without SimCSE: rank 3; no unique coefficients reported','medicine_decomposition_exact':True,
