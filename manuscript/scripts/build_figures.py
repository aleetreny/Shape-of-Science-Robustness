...
def main_figures():
    ...
    fig.subplots_adjust(left=.09, right=.98, bottom=.29, top=.84, wspace=.48)
    ...
    spread = []
    for level, label, color, offset, marker in [
            ('field', '26 Fields', BLUE, -.18, 'o'),
            ('subfield', 'Same 183 Subfields', ORANGE, .18, '^')]:
        selected = [next(a for a in inner if a['population'] == 'same_183_subfields'
        ...
        axes[1].hlines(y, low, high, color=color, linewidth=2, alpha=.55)
        ...
        for a, mean_value, yy in zip(selected, means_inner, y):
            axes[1].annotate(f'{mean_value:.3f}', (mean_value, yy), xytext=(0, -10 if offset > 0 else 6),
                             textcoords='offset points', ha='center', fontsize=6.6, color=color)
            spread.append({'level': level, 'selection': int(a['selection']), 'n': int(a['n']),
                           'mean': mean_value, 'p025': float(a['p025']), 'p975': float(a['p975'])})
        values(item, level, means_inner)
    values(item, 'within_group_spread', spread)
    axes[1].set(yticks=[0, 1, 2], yticklabels=['128', '256', '512'], ylim=(2.6, -.6),
                xlim=(0, 1), xticks=[0, .2, .4, .6, .8, 1], xlabel='Corrected CKA',
                ylabel='Articles per group', title='B  Within groups')
    axes[1].legend(frameon=False, loc='upper left', bbox_to_anchor=(-.14, -.27), fontsize=6.5)
    axes[0].grid(axis='y', alpha=.15); axes[1].grid(axis='x', alpha=.15)
    ...
    points = []
    for higher, label, color, marker in [(True, 'Higher within Subfield (90)', BLUE, '^'),
                                          (False, 'Lower within Subfield (127)', ORANGE, 'o')]:
        keys = [k for k in a if (b[k] > a[k]) == higher]
        assert len(keys) == (90 if higher else 127)
        ax.scatter([a[k] for k in keys], [b[k] for k in keys], s=21, marker=marker,
                   color=color, alpha=.78, label=label, zorder=3)
...
if __name__ == "__main__":
    from build_closure import render
    render(OUT)
