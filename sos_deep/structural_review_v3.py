...
BASE=ROOT/'data/robustness_v2';OUT=BASE/'structural_review_v3'
...
def medicine():
    ...
    for (f,rep,c),counts in groups.items():
        p=np.asarray([v for v in counts.values() if v>0],float);p/=p.sum()
...
def main():
    ...
    with run_lock(OUT):
        ...
        freeze(OUT,['sos_deep/structural_review_v3.py','sos_deep/artifacts.py','sos_deep/control_review.py'],
