...
def run():
    ...
    package = ROOT / 'output/manuscript_source.zip'
    ...
    extracted = ROOT / 'tmp/pdfs/comments_portable_en_source'
    build = ROOT / 'tmp/pdfs/comments_portable_en_build'
    if extracted.exists():
    ...
    for name in ('main', 'supplement'):
        ...
        expected = PdfReader(ROOT / 'output/pdf' / f'{name}.pdf')
        ...
        assert sha(build / f'{name}.pdf') == sha(ROOT / 'output/pdf' / f'{name}.pdf'), name
    ...
    (EVIDENCE / 'portable_en_audit.json').write_text(json.dumps(evidence, indent=2) + '\n')
